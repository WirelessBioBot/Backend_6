...
function SaveUser()
{
  $db = new PDO('mysql:host=localhost;dbname=login', 'login', 'pass',//your login and pass
  [PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);

    $formId = $_COOKIE["id"];

    $agree = empty($_POST["editCheckContract"])?"0":"1";
    $stmt = $db->prepare("UPDATE Data SET fio = :fio, year = :year, email = :email,  gender = :gender, biography = :biography, checkcontract = :checkcontract WHERE id = :id");
